    ...
    <RoleGate minRole="mitglied">
      <div className="flex max-w-2xl flex-col gap-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <StatusPill label={isFull ? "Gesuch voll" : "Offen"} tone={isFull ? "negative" : "positive"} />
        </div>
        ...
        <div className="rounded-lg border bg-card p-5">
          <h2 className="font-serif text-lg font-bold">
            Teilnehmende ({request.participants.length}/{request.maxParticipants})
          </h2>
            ...
            {request.participants.map((participant) => (
              <li key={participant.name} className="flex items-center gap-2.5 text-sm">
                ...
                {participant.name === request.creator.name && (
                  <span className="text-xs text-muted-foreground">(Ersteller)</span>
                )}
              </li>
            ))}
          </ul>
        ...
        <div className="flex gap-3">
          <Button disabled={isFull}>{isFull ? "Gesuch voll" : "Beitreten"}</Button>
