...
import { getMembershipState, requireMeeple } from "@/lib/meeples";
import {
  borrowGame,
  confirmHolding,
  handOverGame,
...
function assertCanReceive(membershipState: ReturnType<typeof getMembershipState>) {
...
export async function scanGiveToMeeple(boardGameId: string, toMeepleId: string) {
...
export async function scanReturnToMeeple(boardGameId: string, toMeepleId: string) {
  const { meeple } = await requireActingMeeple();
...
export async function scanPlaceGameInUnit(boardGameId: string, unitId: string) {
  ...
  return toResult(async () => {
    ...
    return holding.unitId
      ? relocateGame({ boardGameId, toUnitId: unitId, recordedByMeepleId: meeple.id })
      : returnGame({ boardGameId, toUnitId: unitId, recordedByMeepleId: meeple.id });
  });
}
