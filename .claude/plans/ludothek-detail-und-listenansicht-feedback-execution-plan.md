...
- [x] **2. Ribbon-Corner in Grid **und** Liste: Icon entfernen, Badge vergrößern**
...
- [x] **3. Listenzeile deutlich vergrößern**
...
- [x] **4. `RelatedGameCard`: Entfernen-Slot einbauen**
...
- [x] **5. `AssignExpansionDialog` aufteilen: Trigger-Position + Basisspiel-Gate**
...
- [x] **6. Zusammengeführter „Exemplare“-Bereich, ans Seitenende verschoben**
...
- [x] **7. „An Person zurückgeben“ mit Personen-Suche**
...
- [x] **8. Grid, Liste und Kompakt: ein Eintrag pro Titel**
...
- [x] **9. „(x2)“-Anzeige hinter dem Titel**
...
- [x] **10. „Bearbeiten“ öffnet Titel-Dialog, Mängelvermerk wandert ins Aktionen-Menü**
...
- [x] **11. Hover-Beschreibung: kein doppelter Text**
...
- [x] **12. `GameActionsMenu` im Grid + Exemplar-Auswahl-Popup für mehrdeutige Aktionen**
...
- [x] **13. Abschluss: `pnpm run verify` + Zusammenfassung**
...
`pnpm run verify` (format:check + typecheck + lint + test) grün, `pnpm run dup` 0 Klone. Alle 14 Schritte umgesetzt und einzeln committet.

Entscheidungen/Abweichungen während der Umsetzung:

- **Schritt 6 (offene Design-Frage):** wie angenommen "aufklappbar pro Exemplar-Zeile" umgesetzt — ein Akkordeon (`GameCopyTableRow`/`GameCopyCard` in `game-copy-accordion.tsx`) pro Exemplar, das `GameHoldingPanel` + die Historie dieses einen Exemplars zeigt.
- **Schritt 7:** wie empfohlen **ein** Rückgabe-Dialog mit Umschalter "An mich"/"An Person" statt zweier Menüpunkte (`AcceptReturnDialog` in `holding-mini-dialogs.tsx`), um das Menü nicht zu überladen.
- **Schritt 12 (technische Präzisierung, nicht im Plan-Wortlaut):** Um das literale "Popup schließt, Mini-Dialog öffnet sich" zu erreichen, wurde `ActionDialog` (`components/ui/action-dialog.tsx`) um optionale, kontrollierte `open`/`onOpenChange`-Props erweitert (`trigger` ist jetzt optional) — rückwärtskompatibel für alle bisherigen unkontrollierten Aufrufer. `GameActionsMenu` nutzt das, um nach der Exemplar-Auswahl im `CopyPickerDialog` den passenden Mini-Dialog (Ausleihen/Weitergeben/Rückgabe/Umlagern/Mängelvermerk/Deinventarisieren) direkt offen zu rendern.
