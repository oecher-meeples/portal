...
- [x] **2. Schema: `fileName` hinzufügen (#114)**
      ...
      - Abweichung: Migration zunächst von Hand geschrieben (`prisma/migrations/20260813010000_download_file_name/migration.sql`), weil `DATABASE_URL` nur in `.env.local` steht und von `prisma migrate dev` nicht automatisch geladen wird (nur `.env`). Nach Hinweis des Nutzers korrigiert: mit `.env.local` geladen und `pnpm prisma migrate dev` **tatsächlich gegen die Neon-Dev-/Preview-DB ausgeführt** — beide ausstehenden Migrationen angewendet, Backfill von `fileName`/`order` an den 5 realen Zeilen verifiziert (`prisma migrate status` meldet "up to date").
...
- [x] **4. Schema: `order` hinzufügen (#113)**
      ...
      - Abweichung: Wie Schritt 2 — Migration von Hand vorbereitet, dann mit `.env.local` gegen die echte Neon-Dev-/Preview-DB angewendet (`pnpm prisma migrate dev`); Backfill ergab lückenlose `order`-Werte 0–4 passend zur bisherigen `createdAt asc`-Reihenfolge.
