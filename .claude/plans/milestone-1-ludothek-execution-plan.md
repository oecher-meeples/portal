...
- [x] **7. #188 — Sprache erfassen (Sprachneutral-Kennzeichnung + Regelheft-Sprache je Exemplar)**
      Umgesetzt gemäß dem im Issue selbst überarbeiteten Konzept (5-Stufen-BGG-Poll statt Boolean, Mehrfachauswahl statt Einzelwert — die „Getroffene Annahmen" oben waren veraltet, siehe Issue-Kommentare, gleiche Regel wie bei #5): neues Enum `LanguageDependence` (5 Werte, `BoardGame.languageDependence`) und `RuleBookLanguage` (`GameCopy.ruleBookLanguages[]`). BGG-Client parst den `language_dependence`-Poll (meistgewähltes Level als Vorschlag). Import/Abgleich/Formulare (`edit-board-game-title.tsx` Select, `edit-board-game-exemplar.tsx` Checkbox-Gruppe) sowie neue `LanguageIndependentPill` (Level 1) in Card/List-Row/Detailseite; Regelheft-Sprache(n) je Exemplar auf der Detailseite. Kein separater Ludothek-Filter (im Issue nur als „optional" markiert, aus Zeitgründen nicht umgesetzt).
      _Definition of Done:_ Migration angewendet; Formular-Felder speichern korrekt (Unit-Tests für BGG-Parsing, Compare-Diff, Pill-Rendering, Checkbox-Gruppe); `docs/schema.md`/`docs/features.md` aktualisiert.
...
- [x] **10. #198 — Ludothek: CSV-Export des Bestands** — `lib/utils/csv.ts` (`escapeCsvField`/`downloadCsv`) aus `bank-csv-export-dialog.tsx` extrahiert und dort wiederverwendet (DRY). `gameCopyAdminWhere`/`AdminBestandFilter` von `components/feature/admin-bestand/filters.ts` nach `lib/ludothek/admin-bestand-filters.ts` verschoben (Fachlogik gehört in den Domain-Layer). Neue `lib/ludothek/admin-bestand-rows.ts` (`buildAdminBoardGameRows()`, `AdminBoardGameRow`-Typ) bündelt die Row-Query, die zuvor inline in `page.tsx` lag — von Seite und CSV-Export gemeinsam genutzt. Neue `lib/ludothek/bestand-csv.ts` (`buildBestandCsv()`, `exportBestandCsv()`) mit Semikolon-Delimiter (Konsistenz zu `escapeCsvField`/`admin-bank`, nicht Komma wie ursprünglich angenommen). Export-Dialog `admin-bestand-csv-export-dialog.tsx` mit Radio-Auswahl der drei Umfänge, sichtbar nur mit `canManageGames` (aus `hasPermissionInCurrentView(user.id, "games:manage")`, analog `app/ludothek/page.tsx`).
...
- [x] **11. #5 — Lagerverwaltung: Bulk-Scan-Zuordnung (Serienmodus-Feinschliff)** — `scan-view.tsx`: Mehrfachtreffer im Serienmodus „Einlagern" öffnen jetzt denselben Auswahl-Dialog (`select-game`-State mit neuem `forEinlagern`-Flag) statt automatisch alle Exemplare einzusortieren. Kistenwechsel während aktivem Serienmodus (neuer `lastPlacedGame`-State + `confirm-relocate`-View-State) fragt per Ja/Nein nach Umbuchung; beide Pfade machen die neue Kiste zur aktiven. Gleiche-Kiste-Fall unverändert. Neue `relocate-confirm-panel.tsx` ausgelagert (400-Zeilen-Grenze).
...
- [x] **12. #214 — Ludothek: BGG Average Rating speichern und anzeigen (Detailseite + Übersicht)** — Migration `BoardGame.averageRating Float?`. `lib/bgg/client.ts` parst `statistics.ratings.average`. Neue `lib/bgg/rating-scale.ts` (10-stufige Farbskala + deutsche Bedeutung, `resolveRatingScale()` liefert `null` außerhalb 1–10 inkl. `0`-Bewertungen). Fachfreie `components/ui/bgg-rating-hexagon.tsx` nimmt nur aufgelöste Werte entgegen (Layer-Regel: `components/ui` fachlich blind); die Auflösung übernimmt neue `components/entities/bgg-rating-badge.tsx`. Rendering in `game-detail-view.tsx`, `game-card.tsx`, `game-list-row.tsx`. Default-Sortierung in `query.ts` auf `averageRating desc` (Prisma `nulls: "last"`) + `title asc` als Tie-Breaker umgestellt. Compare/Import (`board-game-bgg-compare.ts`, `bgg-compare-panel.tsx`, `board-game-form-values.ts`, `board-games.ts`) analog `weight` erweitert.
